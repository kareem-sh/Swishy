...
def _video_window_size() -> tuple[int, int]:
    """Return configured display size without changing analysis frames."""
    cfg = load_yaml("display.yaml")
    width = max(320, int(cfg.get("window_width", 960)))
    height = max(240, int(cfg.get("window_height", 540)))
    return width, height
...
def run_video_mode(video_path: str) -> dict | None:
    ...
    window_size = _video_window_size()
